 ...
 ** Description: This will run in the background and will decript the message 
...
int main(int argc, char*argv[]){
   ...
   if (bind(listsockfd, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0){// Connect socket to port
      error("ERROR IN SERVER: Cannot bind\n");
   }
   if(BUG){
      fflush(stdout);
...
void runprocess(int nsockfd, int pid){
   ...
   sent = send(nsockfd, "I am the server, and I got your message", 39, 0);
   if (nread < 0){
   ...
   //Recieve key
   kread = recv(nsockfd, key, 12000, 0);
   if(kread < 0){
  ...
  // printf("KEY: %s\n",key);
   for(i=0;i<nread;i++){
      nchar = conint(buffer[i])-conint(key[i]);
	  //printf("nchar: %d\n", nchar);
      if(nchar<0){
		nchar +=27;
      }
